...
writeclass = ("""\
class ActionHelpCode(Action):

    def name(self) -> Text:
        return 'action_help_code'

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ussd_req = tracker.get_slot('helpcode')
        ussdlist = ['broadband','cbc','self_care','pin_registration','daily_usage','club_magnate','mobitel_store','cash_bonanza','registration_details','customer_service']
        requestedService = str(ussd_req)
        ussd_details = {
            ...
            ,'daily_usage' : 'To receive free Daily Usage alerts, type yes & send to 4545 \n To receive an instant usage alert, type usage & send to 4567 '
